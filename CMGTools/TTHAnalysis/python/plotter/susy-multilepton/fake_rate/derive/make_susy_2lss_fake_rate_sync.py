...
PATH="-P /afs/cern.ch/user/p/peruzzi/work/cmgtools/CMSSW_7_4_7/src/CMGTools/TTHAnalysis/python/plotter/susy-multilepton/fake_rate/Spring15_Run2015B_comparison -F sf/t {P}/3_QCDVarsSusy_FakeRateFO_v11/evVarFriend_{cname}.root %s --mcc susy-multilepton/fake_rate/susy_2lss_fake_rate_looseveto_lepchoice.txt %s"
FTREEQCD=" "
FTREETT="  "
OUTDIR='FR/derive'
...
MySel=["ismu","minireliso04","pt10","leppt15","etaLT2p4","isfake","muMediumID","tightcharge","sipLT4","dz01","dxy005","multiiso","isFO1"]
MySel=["ismu","leppt15","multiiso","isFO1"]
...
#    runs.append(["FO1Mu"+"_"+xvar,"susy-multilepton/fake_rate/derive/susy_2lss_fake_rate_perlep.txt",TightMuSel+["nFO1is1","metLT20","mtLT20","jetaway40"],[],[("multiiso","minireliso04")],"-p QCD_Mu ","multiiso",xvar])
...
if doplot:
    for run in runs:
        RUN="python mcPlots.py -f -e --print pdf -l 1.0 --s2v --tree treeProducerSusyMultilepton"
        MYPATH=PATH
        MYPATH = MYPATH % (FTREEQCD if "QCD" in run[5] else FTREETT,"--mcc susy-multilepton/fake_rate/susy_2lss_fake_rate_coneptchoice.txt" if "conept" in run[0] else "--mcc susy-multilepton/fake_rate/susy_2lss_fake_rate_defaultptchoice.txt")
...
#        RUN="python mcEfficiencies.py --yrange 0 1.0 --s2v -l 1.0 --tree treeProducerSusyMultilepton --normEffUncToLumi --outputNumDenHistos"
        RUN="python makeFakeRate.py --yrange 0 1.0 --s2v -l 1.0 --tree treeProducerSusyMultilepton --normEffUncToLumi --outputNumDenHistos"
        MYPATH=PATH
        MYPATH = MYPATH % (FTREEQCD if "QCD" in run[5] else FTREETT,"--mcc susy-multilepton/fake_rate/susy_2lss_fake_rate_coneptchoice.txt" if "conept" in run[0] else "--mcc susy-multilepton/fake_rate/susy_2lss_fake_rate_defaultptchoice.txt")
