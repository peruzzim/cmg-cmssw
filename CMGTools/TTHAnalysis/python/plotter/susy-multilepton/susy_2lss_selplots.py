...
ODIR="test_ra5plots_oct22_mytestFO"
...
for LPt in ['hh','hl','ll']:
    for group in ["","--pgroup 'Fakes + flips':='.*fakes.*','.*flips.*'"]:
        ...
        print "%s %s --sP BR --xp data --pdir /afs/cern.ch/user/p/peruzzi/www/%s/%s%s %s"%(GO,PU_ALL,ODIR,LPt,'_group' if group!='' else '',group)
        ...
        print "%s %s --sP BR --xp data --pdir /afs/cern.ch/user/p/peruzzi/www/%s/%s%s_sortFO %s"%(GO2,PU_ALL,ODIR,LPt,'_group' if group!='' else '',group)
        ...
        print "%s %s --sP SR_%s --xp data --pdir /afs/cern.ch/user/p/peruzzi/www/%s/%s%s %s"%(GO,PU_ALL,ODIR,LPt,LPt,'_group' if group!='' else '',group)
...
print "%s %s --pdir /afs/cern.ch/user/p/peruzzi/www/%s/SS_BR --pgroup 'Fakes + flips':='TT','DY','WJets','Single top'"%(GO,PU_UNBL,ODIR)
