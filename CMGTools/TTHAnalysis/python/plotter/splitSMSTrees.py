...
#    for nev in xrange(t.GetEntries()):
    ...
    for m,elist in allmasses.iteritems():
        ...
        if options.url:
            os.system('echo archiveTreesOnEOS.py --auto -t %s --dset %s %s %s'%(treename,splitdir.split('/')[-1],outdir,options.url))
        os.system('echo rsync -av %s %s'%(splitdir.rstrip('/'),remdir))
        os.system('echo rm %s/%s/tree.root'%(splitdir,treename))
        os.system('echo rm %s/%s/tree.root.url'%(splitdir,treename))
        os.system('echo rm %s/skimAnalyzerCount/SkimReport.pck'%splitdir)
        os.system('echo rm %s/skimAnalyzerCount/SkimReport.txt'%splitdir)
        os.system('echo rmdir %s/%s'%(splitdir,treename))
        os.system('echo rmdir %s/skimAnalyzerCount'%splitdir)
        os.system('echo rmdir %s'%splitdir)
        os.system('echo rmdir %s'%outdir)
