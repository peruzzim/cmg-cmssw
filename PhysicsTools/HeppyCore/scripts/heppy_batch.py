...
def batchScriptCERN( jobDir, remoteDir='', convertLocalToUrl=False):
   '''prepare the LSF version of the batch script, to run on LSF'''
...
""".format(idx=jobDir[jobDir.find("_Chunk")+6:].strip("/"),  srm=(""+remoteDir+jobDir[jobDir.rfind("/"):jobDir.find("_Chunk")]).replace("root://eoscms.cern.ch/",""))
   else:
...
class MyBatchManager( BatchManager ):
   ...
   def PrepareJobUser(self, jobDir, value ):
       ...
       if mode == 'LXPLUS':
           scriptFile.write( batchScriptCERN( jobDir, storeDir, self.options_.convertLocalToUrl) ) 
       elif mode == 'PSI':
